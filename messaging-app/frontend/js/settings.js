...
function generateSettings() {
    ...
    const profileContainer = $('<div>', { class: 'profile-container h-28 flex items-center my-6' });
    ...
    const listSettings = $('<div>', { class: 'list-settings flex h-96' });
    ...
    const settingsItems = [
        ...
        { imgSrc: '../assets/svg/logout.svg', text: 'Cerrar sesión', imgClass: 'h-14 ml-1' }
    ];

    settingsItems.forEach(item => {
        const button = $('<button>');
        const listItem = $('<li>', { class: 'flex items-center' });
        const img = $('<img>', { class: item.imgClass || 'h-14', src: item.imgSrc });
