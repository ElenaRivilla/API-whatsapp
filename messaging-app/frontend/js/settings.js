...
export function accountSettings() {
    ...
    const label = $("<label>").addClass("relative inline-block w-20 h-8").text("Cambiar modo:");
    const inputCheck = $("<input>").addClass("sr-only peer").attr("type", "checkbox");
    const modeDiv = $("<div>").addClass("w-full h-full bg-gray-400 rounded-full cursor-pointer transition duration-300 peer-checked:bg-blue-600 peer-checked:before:translate-x-12 peer-checked:before:bg- before:content-[''] before:absolute before:top-1 before:left-1 before:bg-white before:w-6 before:h-6 before:rounded-full before:transition before:duration-300");
    
    label.append(inputCheck, modeDiv);
    accountContainer.append(title, nameLabel, bioLabel, label);
