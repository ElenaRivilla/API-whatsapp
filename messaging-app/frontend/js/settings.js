...
export function generateSettings(user) {
    ...
    const backButton = $('<button>').addClass(`back-button p-2 rounded-full h-12 w-12 flex items-center justify-center`).attr({
        "style": "background-color: var(--send-button-color);",
        "aria-label": "Back"
    });
    const backLink = $('<a>');
    const backImg = $('<img>').addClass('contrastIcon h-5').attr({
    ...
    const settingsTitle = $("<h3>").addClass("text-xl ml-4").text("Ajustes").attr("aria-label", "Settings");
    ...
    const profileImg = $('<img>').addClass('h-16 rounded-full').attr({
        'src': user.image,
        "alt": "user image"
    });
    const profileName = $('<h2>').addClass('text-xl font-bold w-42').text(user.username).attr("aria-label", "Username");
    const profileBio = $("<p>").addClass("text-sm w-56 sm:w-62 md:min-w-30 truncate").text(user.bio).attr("aria-label", "User bio");
    ...
    settingsItems.forEach(item => {
        const button = $("<button>").attr("aria-label", item.text);
        const listItem = $("<li>").addClass("flex items-center").attr({ "text": item.text, 'id': item.id });
        const img = $("<img>").addClass(item.imgClass || "h-10").attr({
            "src": item.imgSrc,
...
export function accountSettings(user) {
    ...
    const profileTitle = $('<h2>').addClass('font-bold 2xl:text-xl xl:text-lg lg:text-base').text('Foto de perfil').attr("aria-label", "Profile photo");
    const profileSeparator = $('<hr>').addClass(`my-3 border-t-2 border-[${backgroundColor}] w-[65%]`);
    const profileImg = $('<img>').addClass('rounded-full 2xl:h-44 mb-4 xl:h-36 lg:h-28 md:h-20').attr({
        'src': user.image,
        "alt": "user image"
    });
    ...
    const labelName = $('<label>').addClass('my-4').attr("aria-label", "Name");
    ...
    const inputName = $('<input>').addClass(`input-name rounded-full focus:outline-none focus:ring-2 focus:ring-[${accentColor}] focus:border-[${accentColor}] h-10 w-[90%]`).attr({
        'type': 'text',
        'placeholder': 'Escribe tu nombre..',
        "style": "background-color: var(--typebar-color)"
    }).val(user.username);
    ...
    const textBio = $('<textarea>').addClass(`text-bio rounded-full focus:outline-none focus:ring-2 focus:ring-[${accentColor}] focus:border-[${accentColor}] h-10 w-[90%] resize-none overflow-y-auto scrollbar-hide`)
        .attr({
            ...
            "style": "background-color: var(--typebar-color); max-height: 80px;"
        }).val(user.bio);
    ...
    const sendButtonElement = $('<button>').addClass(`send-button btn bg-[${sendButton}] hover:bg-[${accentColor}] text-white py-2 px-4 rounded-full`).text('Guardar cambios').attr("aria-label", "Save changes");
...
export function chatSettings() {
    ...
    const title = $("<h2>").addClass("text-2xl font-bold mb-4").text("Configuración de Chats").attr("aria-label", "Chat settings");

    const label = $("<label>").addClass("relative inline-block w-20 h-8").attr("aria-label", "Change mode");
